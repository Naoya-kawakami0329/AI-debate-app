...
export async function saveDebateAction(debateState: DebateState): Promise<{ success: boolean; debateId?: string; error?: string }> {
  try {
    ...
    if (debateId) {
      ...
      return { success: false, error: 'Database service returned null - check Supabase configuration' };
    }
  } catch (error) {
...
export async function getRecentDebatesAction(limit: number = 10) {
  try {
    ...
    return { success: false, debates: [], error: error instanceof Error ? error.message : 'Unknown error' };
  }
}

export async function addAudienceQuestionAction(debateId: string, question: string, author: string = 'Anonymous') {
  try {
    const success = await DatabaseService.addAudienceQuestion(debateId, question, author);
    if (success) {
      revalidatePath('/');
    }
    return { success };
