import { DebateState, DebateMessage, DebateStage, AIModel, Evidence } from './types';
...
export class DebateEngine {
  ...
  async generateMessage(stage: DebateStage, speaker: 'pro' | 'con'): Promise<DebateMessage> {
    const model = speaker === 'pro' ? this.debateState.config.proModel : this.debateState.config.conModel;
    ...
    do {
      ...
      if (this.useRealAI) {
        try {
          console.log(`Calling real AI API for ${model.provider} (attempt ${attempts})...`);
          ...
          console.error('Failed to generate AI message, falling back to mock:', {
      ...
      const isDuplicate = this.debateState.messages.some(msg => 
      ...
      console.log(`Duplicate content detected (attempt ${attempts}), regenerating...`);
      
      if (attempts >= maxAttempts) {
        console.warn('Max attempts reached, using content with slight modification');
  ...
  private async generateRealAIMessage(stage: DebateStage, speaker: 'pro' | 'con', model: AIModel): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || `HTTP ${response.status}: Failed to generate AI message`);
    }
  ...
  private generateMockMessage(stage: DebateStage, speaker: 'pro' | 'con', model: AIModel): string {
  ...
  private async searchEvidence(content: string, speaker: 'pro' | 'con'): Promise<Evidence[]> {
  ...
  async nextStage(): Promise<DebateStage> {
    const stageOrder: DebateStage[] = ['setup', 'opening', 'rebuttal', 'closing', 'summary'];
