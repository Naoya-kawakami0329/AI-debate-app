...
export default function MessageCard({ message }: MessageCardProps) {
  ...
  return (
    <Card className={`${isProSide ? 'ml-0 mr-8' : 'ml-8 mr-0'} transition-all duration-300 hover:shadow-md`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className={`flex-1 ${isProSide ? 'order-2' : 'order-2'}`}>
            <div className="flex items-center gap-2 mb-2">
              <Badge variant={isProSide ? 'default' : 'destructive'} className="text-xs">
                {message.model.name}
              </Badge>
            ...
            {message.evidence.length > 0 && (
              <div className="space-y-2">
                {message.evidence.map((evidence) => (
                  ...
                  >
                    <div className="font-medium text-blue-600">{evidence.title}</div>
